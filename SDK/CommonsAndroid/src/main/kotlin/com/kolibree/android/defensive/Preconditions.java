...
public class Preconditions {
  ...
  @VisibleForTesting public static Preconditions INSTANCE = new Preconditions();
  ...
  /**
   * USE ONLY AS A LAST RESORT SOLUTION Added cause some Espresso tests need to force entities with
   * values that break preconditions
   */
  @VisibleForTesting
  public static void setEnabled(Boolean enabled) {
    Preconditions.enabled = enabled;
  }
  ...
  public static @NonNull <T extends CharSequence> T checkStringNotEmpty(final T string) {
    if (enabled && TextUtils.isEmpty(string)) {
      throw new IllegalArgumentException();
    }
    return string;
  }
  ...
  public static int checkFlagsArgument(final int requestedFlags, final int allowedFlags) {
    if (enabled && (requestedFlags & allowedFlags) != requestedFlags) {
      throw new IllegalArgumentException(
          "Requested flags 0x"
