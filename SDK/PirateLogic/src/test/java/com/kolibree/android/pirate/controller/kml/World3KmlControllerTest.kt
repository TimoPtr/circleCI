...
import com.kolibree.android.pirate.controller.World3Constant
...
class World3KmlControllerTest : BaseUnitTest() {

    private lateinit var worldController: World3KmlController
    ...
    private val callback = mock<SupervisedCallback16>()

    @Before
    fun setUp() {
        worldController = spy(World3KmlController(true, checkupCalculator, appContext, pirateHelper))
    ...
    fun `setPrescribedZoneId sets currentZone and waitingForChangeLaneMessage to true`() {
        worldController.setPrescribedZoneId(World3Constant.KLPirateLevel3PrescribedZone.KLPirateLevel3PrescribedZoneIncIntBottom.ordinal)

        assertTrue(worldController.waitingForChangeLaneMessage.get())
        assertEquals(MouthZone16.LoIncInt, worldController.currentZone.get())
    }
    ...
    fun `onRawData invokes addRawData with Running when shouldContinueProcessingData is true and currentZone is set and isPlaying true`() {
        val expectedZone = MouthZone16.LoMolRiInt
