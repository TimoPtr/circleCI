...
class World1KmlControllerTest : BaseUnitTest() {

    private lateinit var worldController: World1KmlController

    private val checkupCalculator = mock<CheckupCalculator>()

    private val appContext = mock<SupervisedBrushingAppContext8>()

    private val pirateHelper = mock<PirateHelper>()

    private val callback = mock<SupervisedCallback8>()

    @Before
    fun setUp() {
        worldController = spy(World1KmlController(true, checkupCalculator, appContext, pirateHelper))
    ...
    @Test
    fun `setPrescribedZoneId sets currentZone`() {
        worldController.setPrescribedZoneId(World1Constant.KLPirateLevel1PrescribedZone.KLPirateLevel1PrescribedZoneLeftExtBottom.ordinal)

        assertEquals(MouthZone8.LoLeExt, worldController.currentZone.get())
    }
    ...
    fun `onRawData invokes addRawData with Running when shouldContinueProcessingData is true and currentZone is set and isPlaying true`() {
        val expectedZone = MouthZone8.UpRiInt
