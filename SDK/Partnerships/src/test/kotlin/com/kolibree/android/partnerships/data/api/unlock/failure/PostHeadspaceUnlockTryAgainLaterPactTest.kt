...
package com.kolibree.android.partnerships.data.api.unlock.failure
...
class PostHeadspaceUnlockTryAgainLaterPactTest :
    ...
    override fun runTest(mockServer: MockServer, context: PactTestExecutionContext) {
        ...
        assertEquals(503, apiError.httpCode)
        assertEquals(115, apiError.internalErrorCode)
        assertEquals("Discount code not available, please try again later.", apiError.details)
    }
    ...
    private val expectedError = newJsonBody { root ->
        root.stringValue("display_message", "Something went wrong.")
            ...
            .stringValue("detail", "Discount code not available, please try again later.")
            .numberValue("http_code", 503)
    }.build()
}
