...
internal class AddDirtyColumnMigrationTest :
    ...
    fun migration_addsDirtyColumnWithValueZero() {
        ...
        initializeDatabaseWith(schemaVersion = 2) {
            val insertQuery =
                "INSERT OR REPLACE INTO `$TABLE_NAME` (`mac`,`name`,`model`,`account_id`,`profile_id`,`serial`,`hardware_version`,`firmware_version`,`bootloader_version`) VALUES ('%s','%s','%s',%s,%s,'%s',%s,%s,%s)"
            ...
            queryAccountToothbrush().use { cursor ->
                ...
                assertEquals(-1, cursor.getColumnIndex("dirty"))
            }
        }

        runMigrationAndCheck(AddDirtyColumnMigration) {
            queryAccountToothbrush().use { cursor ->
                cursor.apply {
