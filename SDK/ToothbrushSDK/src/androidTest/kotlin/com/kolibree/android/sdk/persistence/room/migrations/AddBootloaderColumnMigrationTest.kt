...
internal class AddBootloaderColumnMigrationTest :
    ...
    @Test
    fun migration_addsBootloaderColumnWithValueZero() {
        val converters = AccountToothbrushConverters()
        initializeDatabaseWith(schemaVersion = 1) {
            val insertQuery =
                "INSERT OR REPLACE INTO `$TABLE_NAME` (`mac`,`name`,`model`,`account_id`,`profile_id`,`serial`,`hardware_version`,`firmware_version`) VALUES ('%s','%s','%s',%s,%s,'%s',%s,%s)"
            ...
            queryAccountToothbrush().use { cursor ->
                ...
                assertEquals(-1, cursor.getColumnIndex("bootloader_version"))
            }
        }

        runMigrationAndCheck(AddBootloaderColumnMigration) {
            queryAccountToothbrush().use { cursor ->
                cursor.apply {
                    ...
                    val bootloaderVersion = longValueForColumn("bootloader_version")
                    assertEquals(0L, bootloaderVersion)

                    assertEquals(
                        SoftwareVersion.NULL,
                        converters.toSoftwareVersion(bootloaderVersion!!)
                    )
                }
            }
        }
